...
class AsyncBasePermission(Protocol):
    async def has_permission(access_policy: AccessPolicy, request: HttpRequest, auth_result: AuthResult, handler: Callable[..., Any]) -> bool:
        """
        Return True if the user has permission to access the handler, otherwise return False.
        """
        ...
...
async   def run_auth(access_policy: AccessPolicy, request: HttpRequest) -> AuthResult | None:
...
async def is_have_credentials(backend: type[BaseAuthentication | AsyncBaseAuthentication], request: HttpRequest) -> bool:
...
async def is_authenticate(backend: type[BaseAuthentication | AsyncBaseAuthentication], request: HttpRequest) -> AuthResult | None:
...
async def run_permissions(access_policy: AccessPolicy, request: HttpRequest, auth_result: AuthResult, handler: Callable[..., Any]) -> bool:
